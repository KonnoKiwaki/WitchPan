...
import {getFolderInfo, getShareFolderInfo} from '@/api/file'
...
function openFolder(data, shareId) {
...
// 获取当前路径的目录
const getNavigationFolder = async (folderInfo) => {
  console.log(folderInfo);
  try {
    const res = await axios.get(`/api/showShare/getFolderInfo`, {
      params: {
        path: folderInfo.path,
        shareId: folderInfo.shareId
      }
    });
    console.log(res)
    if (res.data) {
...
watch(
    ...
    (newVal, oldValue) => {
      ...
      if (newVal.path.indexOf('/share') === -1 ){
        return
      }
      ...
      if (path == undefined) {
        ...
        if (folderList.value.length == 0) {
          folderInfo.shareId = newVal.path.split('/').pop()
          folderInfo.path = path
          // 没有目录信息，发请求获取
          getNavigationFolder(folderInfo)
        }
