...
<!--    <div class="bg"></div>-->
    <div class="login-panel">
      <el-form :model="formData" :rules="rules" ref="loginFormRef" class="login-register">
        <div class="login-title">魔女云盘</div>
        ...
        <div  class="login-btn-qq" v-if="opType == 1">
          <span @click="qqLogin" class="clickable">快捷登陆</span>
          <img @click="qqLogin" class="clickable" src="https://passport.baidu.com/passApi/img/bd-acc-qzone.png" />
        </div>
      </el-form>
    </div>
    <Dialog
...
//TODO
//qq验证以后，官方传递一个code 前端接收，然后发送给后端，后端处理后，将显示信息传给前端，前端来显示

const qqLogin = async () => {
  try {
    // 向后端发送请求，获取 QQ 登录的信息
    const response = await axios.post(api.qqLogin, {
      // 如果需要，你可以在这里发送一些数据到后端
    });

    // 使用获取的信息构建 QQ 登录的 URL
    const qqLoginInfo = response.data;

    // 使用构建的 URL 重定向用户
    window.location.href = qqLoginInfo;
  } catch (error) {
    // 请求失败后的处理
    console.error(error);
  }
...
.login-body {
  ...
  background-image: url('../assets/login_witch.jpg');
  .bg {
    flex: 1;
    background-size: cover;
